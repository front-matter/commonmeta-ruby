  ...
  module Readers
    module DataciteReader
      ...
      def read_datacite(string: nil, **_options)
        ...
        type = Briard::Utils::DC_TO_CM_TRANSLATIONS.fetch(resource_type_general, "Other")
        additional_type = Briard::Utils::DC_TO_CM_TRANSLATIONS.fetch(resource_type, nil)
        ...
        date = {}
        date["created"] = get_iso8601_date(meta.dig("created")) || get_date(meta.dig("dates"), "Created")
        date["published"] = get_iso8601_date(meta.dig("published")) || get_date(meta.dig("dates"), "Issued") || get_iso8601_date(meta.dig("publication_year"))
        date["registered"] = get_iso8601_date(meta.dig("registered"))
        date["updated"] = get_iso8601_date(meta.dig("updated")) || get_date(meta.dig("dates"), "Updated")
        ...
        references = (Array.wrap(meta.fetch("related_identifiers", nil)) + Array.wrap(meta.fetch("related_items", nil))).select do |r|
      ...
      def format_contributor(contributor)
        ...
        { "name" => type != "Person" ? contributor.fetch("name", nil) : nil,
          "type" => type,
