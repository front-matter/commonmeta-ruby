...
module Briard
    module Readers
      module CrossrefJsonReader
        def get_crossref_json(id: nil, **options)
        ...
        def read_crossref_json(string: nil, **options)
          if string.present?
          ...
          container = if meta.fetch('container-title', nil).present?
                        ...
                        { 'type' => container_type,
                          'title' => parse_attributes(meta.fetch('container-title', nil), first: true).to_s.squish,
                          'identifier' => issn,
