...
describe Commonmeta::Metadata, vcr: true do
  ...
  context "get crossref metadata" do
    ...
    it "journal article" do
      ...
      expect(subject.type).to eq("JournalArticle")
      expect(subject.creators.length).to eq(5)
      expect(subject.creators.first).to eq("givenName" => "Markus", "familyName" => "Ralser", "type" => "Person")
    ...
    it "journal article with RDF for container" do
      ...
      expect(subject.date).to include("registered" => "2015-09-25",
                                      "updated" => "2019-07-05")
    ...
    it "peer review" do
      ...
      expect(subject.date).to include("registered" => "2020-05-19",
                                      "updated" => "2020-05-19")
      expect(subject.publisher).to eq("name" => "eLife Sciences Publications, Ltd")
      expect(subject.provider).to eq("Crossref")
    end
    ...
    it "component" do
      ...
      expect(subject.descriptions.empty?).to be(true)
      expect(subject.date).to eq("registered" => "2015-10-16",
                                 "updated" => "2018-10-19")
      expect(subject.publisher).to eq("name" => "Public Library of Science (PLoS)")
      expect(subject.provider).to eq("Crossref")
    end
    ...
    it "book chapter" do
      ...
      expect(subject.type).to eq("BookChapter")
      expect(subject.creators.length).to eq(2)
      expect(subject.creators.first).to eq("type" => "Person", "givenName" => "Ronald L.", "familyName" => "Diercks")
    ...
    it "yet another book chapter" do
      ...
      expect(subject.date).to eq("registered" => "2018-11-19",
                                 "updated" => "2019-07-02")
    ...
    it "yet another book" do
      ...
      expect(subject.date).to eq("published" => "1982",
                                 "updated" => "2021-12-04")
      expect(subject.publisher).to eq("name" => "Wiley")
    ...
    it "JaLC" do
      ...
      expect(subject.date).to eq("published" => "1997", "registered" => "2002-08-08", "updated" => "2020-03-06")
    ...
    it "OP" do
      ...
      expect(subject.creators.first).to eq("familyName" => "Younes", "givenName" => "Maged", "type" => "Person")
    ...
    it "invalid date" do
      ...
      expect(subject.date).to eq("published" => "2020-04-08",
                                 "updated" => "2020-06-16")
    ...
    it "journal issue" do
      ...
      expect(subject.creators).to eq([{ "name" => ":(unav)", "type" => "Organization" }])
      expect(subject.titles).to eq([{ "title" => ":(unav)" }])
      expect(subject.date).to eq("published" => "2018-10",
                                 "updated" => "2018-10-03")
