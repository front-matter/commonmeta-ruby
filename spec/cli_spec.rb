...
describe Commonmeta::CLI do
  ...
  describe 'convert from id', vcr: true do
    SCHEMA_ORG_REGEX = %r{"@context": "http://schema.org"}
    CROSSREF_XML_REGEX = /<doi_batch/
    DATACITE_REGEX = /"types": {/
    BIBTEX_REGEX = %r{@article{https://doi.org/10.7554/elife.01567}
    BIBTEX_DATA_REGEX = %r{@misc{https://doi.org/10.5061/dryad.8515}
    RIS_REGEX = /TY  - JOUR/
  ...
  describe 'convert file' do
    ...
    context 'datacite', vcr: true do
      ...
      it 'to bibtex' do
        subject.options = { to: 'bibtex' }
        expect { subject.convert file }.to output(%r{@article{https://doi.org/10.5438/4k3m-nyvg}).to_stdout
      end
      ...
      it 'to datacite invalid' do
        ...
        expect {
 subject.convert file }.to output(/root is missing required keys: id, type, url, creators, titles, publisher, date/).to_stderr
      end
    ...
    context 'codemeta' do
      ...
      it 'to bibtex' do
        subject.options = { to: 'bibtex' }
        expect { subject.convert file }.to output(%r{@misc{https://doi.org/10.5063/f1m61h5x}).to_stdout
      end
    end
  ...
  describe 'encode' do
    ...
    it 'blog prefix' do
      expect { subject.encode input }.to output(%r{https://doi.org/10.53731}).to_stdout
    end
  end
